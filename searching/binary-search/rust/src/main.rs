...
/// Finds an element in an array using Binary Search.
fn binary_search<T: PartialOrd>(array: &[T], x: &T) -> usize {
    let mut low: usize = 0;
...
fn test(length: usize, verbose: bool) {
    ...
    let index: usize = binary_search(&array, key);
    let end: Instant = Instant::now();

    // Report time.
    println!("Binary Search: {:.2?} us", (end - start).as_micros());
...
fn usage(args: &Vec<String>) {
    println!("{} - Testing program for binary search.", args[0]);
